...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/09 19:37:37 by lkomarov          #+#    #+#             */
/*   Updated: 2017/01/09 20:32:48 by lkomarov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_lt(char type, int i) /*checks the length and the type*/
{
    char *check_str;
...
void	ft_fouth_length(const char *format) /*checks the length ll and hh*/
{
...
void	ft_third_precision(const char *format)
{
    ...
    {
        g_str->star_pr = 1;
        IF_NBR(*(format + 1)) ? (g_str->star_pr++, check++) : check;
        i++;
    }
    if (IF_NBR(*format) || check == 1 || *format == '0')
    {
        ...
        (*(format + i) == '*' && check != 1) ? (g_str->star_pr = 1, i++) : i;
    }
